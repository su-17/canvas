...
function loadSettings() {
    ...
    filter3checkbox = document.getElementById('filter4checkbox');
...
function filter1() {
    ...
    if (isLoaded()) {
        clear();
        if (!filter1checkbox.checked || filter2checkbox.checked || filter3checkbox.checked) {
            filter3checkbox.checked = false;
...
function filter2() {
    ...
    if (isLoaded()) {
        clear();
        if (!filter2checkbox.checked || filter1checkbox.checked || filter3checkbox.checked) {
            filter1checkbox.checked = false;
            filter3checkbox.checked = false;
        }
        if (filter2checkbox.checked) {
...
function filter3() {
    loadSettings();
    if (isLoaded()) {
        if (!filter3checkbox.checked || filter2checkbox.checked || filter1checkbox.checked) {
            filter1checkbox.checked = false;
            filter2checkbox.checked = false;
...
function printCont() {
    ...
    var afryka = new Image();
    australia.src = "img/australia.png";
    afryka.src = "img/afryka.png";
    australia.onload = function () {
        ctx.drawImage(australia, loadedimg.width / 3 - 100, 800, 100, 100);
        ctx.drawImage(australia, loadedimg.width / 5 - 200, loadedimg.height - 800, 100, 200);
    };
    afryka.onload = function () {
        ctx.drawImage(afryka, 100, loadedimg.height / 2, 50, 50);
        ctx.drawImage(afryka, loadedimg.width - 150, loadedimg.height / 2, 50, 50);
    }

}

function printKleks() {
    var kleks1 = new Image();
    var kleks2 = new Image();
    kleks1.src = "img/kleks1.png";
    kleks2.src = "img/kleks2.png";
    kleks1.onload = function () {
        ctx.drawImage(kleks1, loadedimg.width / 3 - 200, 800, 100, 100);
        ctx.drawImage(kleks1, loadedimg.width / 5 - 300, loadedimg.height - 1000, 100, 400);
    };
    kleks2.onload = function () {
        ctx.drawImage(kleks2, 100, loadedimg.height / 2,100, 50);
        ctx.drawImage(kleks2, loadedimg.width - 250, loadedimg.height / 2, 80, 50);
    }
